...
public:
    ...
    float fMaxDistance = sqrt(map.nMapX * map.nMapX + map.nMapY * map.nMapY);
...
#define GATE_BLOCK  'G'    // has height 1
    ...
    bool OnUserCreate() override {
        ...
        map.fMap = new float[map.nMapX * map.nMapY];
        for (int y = 0; y < map.nMapY; y++) {
            for (int x = 0; x < map.nMapX; x++) {
                switch (map.sMap[y * map.nMapX + x]) {
                case GRND_FLOOR: map.fMap[y * map.nMapX + x] = 0.0f; break;
                case FRST_FLOOR: map.fMap[y * map.nMapX + x] = 1.0f; break;
                case SCND_FLOOR: map.fMap[y * map.nMapX + x] = MULTIPLE_LEVELS ? 2.0f : 1.0f; break;
                case THRD_FLOOR: map.fMap[y * map.nMapX + x] = MULTIPLE_LEVELS ? 3.0f : 1.0f; break;
                case FRTH_FLOOR: map.fMap[y * map.nMapX + x] = MULTIPLE_LEVELS ? 4.0f : 1.0f; break;
                case FFTH_FLOOR: map.fMap[y * map.nMapX + x] = MULTIPLE_LEVELS ? 5.0f : 1.0f; break;
                case SXTH_FLOOR: map.fMap[y * map.nMapX + x] = MULTIPLE_LEVELS ? 6.0f : 1.0f; break;

                case FLOOR_1QRTR: map.fMap[y * map.nMapX + x] = 0.25f; break;
                case FLOOR_HALVE: map.fMap[y * map.nMapX + x] = 0.50f; break;
                case FLOOR_3QRTR: map.fMap[y * map.nMapX + x] = 0.75f; break;
                }
            }
        }
        ...
        pWallSprite = load_sprite_file("stonewall.png"); bSuccess &= (pWallSprite != nullptr);
        pFloorSprite = load_sprite_file( "sand1.png"); bSuccess &= (pFloorSprite != nullptr);
        pCeilSprite = load_sprite_file("sand.png"); bSuccess &= (pCeilSprite != nullptr);
        pRoofSprite = load_sprite_file("Tatooinehousewall.png"); bSuccess &= (pRoofSprite != nullptr);
        sprite.initSprites(ScreenWidth());
    ...
    bool GetDistancesToWalls(float fRayAngle, std::vector<IntersectInfo>& vHitList) {
        ...
        while (!bOutOfBounds && !bDestCellFound && fDistIfFound < fMaxDistance) {
            ...
            if (bOutOfBounds) {
                ...
                bool bHitFound = (map.fMap[nCurY * map.nMapX + nCurX] != fCurHeight);
                bDestCellFound = (nCurX == int(fToX) && nCurY == int(fToY));

                if (bHitFound) {

                    // reset current height to new value
                    fCurHeight = map.fMap[nCurY * map.nMapX + nCurX];
    ...
    bool OnUserUpdate(float fElapsedTime) override {
        ...
        if (fNewX >= 0 && fNewX < map.nMapX &&
            ...
            float(map.fMap[int(fNewY) * map.nMapX + int(fNewX)]) < player.fPlayerH) {
        ...
        if (MULTIPLE_LEVELS) {
            ...
            if (GetKey(olc::PGDN).bHeld) {
                ...
                if (fNewHeight > 0.0f && float(map.fMap[int(player.fPlayerY) * map.nMapX + int(player.fPlayerX)]) < fNewHeight) {
        ...
        int nHorizonHeight = ScreenHeight() * player.fPlayerH + (int)player.fLookUp;
        float fAngleStep = player.fPlayerFoV_deg / float(ScreenWidth());
...
#define    ROOF_DRAWING 4
        ...
        //for (int x = 0; x < ScreenWidth(); x++)
        //{
        //    sprite.DrawSprites(this, x, fMaxDistance, player, map, fElapsedTime);
        //}

        // output player and rendering values for debugging
        DrawString(10, 10, "fPlayerX = " + std::to_string(player.fPlayerX), TEXT_COLOUR);
        DrawString(10, 20, "fPlayerY = " + std::to_string(player.fPlayerY), TEXT_COLOUR);
        DrawString(10, 30, "fPlayerA = " + std::to_string(player.fPlayerA_deg), TEXT_COLOUR);
        DrawString(10, 40, "fPlayerH = " + std::to_string(player.fPlayerH), TEXT_COLOUR);
        DrawString(10, 50, "fLookUp  = " + std::to_string(player.fLookUp), TEXT_COLOUR);
